...
# Mendefinisikan fungsi utama untuk mengolah data supplier
def supplier_funnel():
    ...
    try:
        ...
        # Membaca file Excel berisi data supplier dan menyimpannya ke dalam tabel 'supplier' di PostgreSQL
        df = pd.read_excel("data/supplier.xls")
        df.to_sql("supplier", engine, if_exists="replace", index=False)
        
        # Jika eksekusi sampai di sini tanpa error, maka dianggap berhasil
        print("Data supplier berhasil dimasukkan ke dalam tabel 'supplier'.")
...
# Mendefinisikan tugas (task) untuk mengolah data supplier menggunakan PythonOperator
task_load_supplier = PythonOperator(            
     task_id="ingest_supplier",      # ID tugas
     python_callable=supplier_funnel, # Fungsi Python yang akan dijalankan
